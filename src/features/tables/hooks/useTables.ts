...
export function usePayPartialOrder() {
  ...
  return useMutation(
    ({ orderId, amount, paymentMethod }: { orderId: number; amount: number; paymentMethod: string; tableId: number }) =>
...
export function useChangeOrderTable() {
  ...
  return useMutation(
    ({ orderId, destinationTableId }: { orderId: number; destinationTableId: number }) =>
